.. index:: pair: page; Discrete Fourier Transformation for real-valued input (RDFT)
...
:target:`doxid-openvino_docs_ops_signals__r_d_f_t_9_1md_openvino_docs_ops_signals_rdft_9` **Versioned name** : *RDFT-9*
...
#. Calculate ``normalized_axes``, where each ``normalized_axes[i] = axes[i]``, if ``axes[i] >= 0``, and ``normalized_axes[i] = axes[i] + r`` otherwise.
...
.. math::
	...
	Y[n_0,\dots,n_{k-1},m_0,\dots,m_{q-1}]=\sum\limits_{p_0=0}^{S_0}\cdots\sum\limits_{p_{q-1}=0}^{S_{q-1}}X[n_0,\dots,n_{k-1},j_0,\dots,j_{q-1}]\exp\left(-2\pi i\sum\limits_{b=0}^{q-1}\frac{m_bj_b}{S_b}\right)
