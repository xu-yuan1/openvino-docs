.. index:: pair: page; ReduceMin
.. _doxid-openvino_docs_ops_reduction__reduce_min_1:


ReduceMin
=========

:target:`doxid-openvino_docs_ops_reduction__reduce_min_1_1md_openvino_docs_ops_reduction_reducemin_1` **Versioned name** : *ReduceMin-1*
...
*ReduceMin* operation performs the reduction with finding the minimum value on a given input ``data`` along dimensions specified by ``axes`` input. Each element in the output is calculated as follows:
...
#. If ``axes`` is an empty list, *ReduceMin* corresponds to the identity operation.
