.. index:: pair: page; ReduceMax
.. _doxid-openvino_docs_ops_reduction__reduce_max_1:


ReduceMax
=========

:target:`doxid-openvino_docs_ops_reduction__reduce_max_1_1md_openvino_docs_ops_reduction_reducemax_1` **Versioned name** : *ReduceMax-1*
...
*ReduceMax* operation performs the reduction with finding the maximum value on a given input ``data`` along dimensions specified by ``axes`` input. Each element in the output is calculated as follows:
...
#. If ``axes`` is an empty list, *ReduceMax* corresponds to the identity operation.
