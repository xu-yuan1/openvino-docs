...
.. _dl_workbench__troubleshooting:

.. meta::
   :description: Trobuleshooting of OpenVINO DL Workbench. Information about how 
                 to investigate and troubleshoot most frequent issues with DL Workbench.
   :keywords: Deep Learning Workbench, DL Workbench, DL Workbench troubleshooting, OpenVINO, 
              DevCloud, investigate issues, DL Workbench UI, Docker Container Stops, 
              Incompatible Model and Dataset, Wrong archive, Open Model Zoo models do not get imported, 
              Nginx fails to start, Unable to upgrade the DL Workbench, warning, No sudo privileges, 
              GPU drivers setup fails, Authentication Error, failures, Python, pip, os, version not supported


Troubleshooting
===============

:target:`dl_workbench__troubleshooting_1md_openvino_workbench_docs_workbench_dg_troubleshooting`
...
To learn more about an error, download a ``.txt`` file with server logs. Click the *user icon* in the upper-right corner 
to see the **Settings**, then click **Download Log** :
...
.. note::
   Server logs contain sensitive information like data on your models. If you do not wish to share this information, 
   attach only the ``RUN COMMAND`` part.
...
#. Copy required DL Workbench assets into it. Assign this path to the ``--assets-directory`` argument in the script you used to :ref:`install the application <dl_workbench__install>`.
...
Also, check that you do not select a VOC Object-Detection dataset for a Classification model, or an ImageNet Classification 
dataset for an Object-Detection model.
...
connection and specify your proxy settings.
...
`hostname <https://en.ryte.com/wiki/Hostname#:~:text=A%20hostname%20is%20a%20unique,multiple%20domains%20under%20one%20host.>`__ of your machine or its `IPv4 address <https://en.wikipedia.org/wiki/IPv4#:~:text=Internet%20Protocol%20version%204%20(IPv4,in%20the%20ARPANET%20in%201983.)>`__.
...
Check the `user name <https://www.google.com/amp/s/www.cyberciti.biz/faq/appleosx-bsd-shell-script-get-current-user/amp/>`__ 
